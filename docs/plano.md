...
| Estilo | **Kit `branding/` como está** (classes `rc-*`, tokens, tema escuro) + **CSS Modules**. **Sem Tailwind.** Fontes via `next/font/google` (Fraunces + DM Sans). Ícones `lucide-react`. | Um só vocabulário, bundle mínimo, tema escuro por `data-theme` continua funcionando. |
