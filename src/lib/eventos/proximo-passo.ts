...
const prazo = (dias: number) => (dias === 0 ? "hoje" : dias === 1 ? "amanhã" : `em ${dias} dias`);
...
export function proximoPasso(s: SituacaoEvento, agora: Date = new Date()): string {
  ...
  if (s.status.aberto) return `Inscrições encerram ${prazo(diasAte(s.inscricoes_fim, agora))}.`;
  if (s.status.motivo === "antes_do_periodo") return `Inscrições abrem ${prazo(diasAte(s.inscricoes_inicio, agora))}.`;
  ...
  return `Montagem confirmada. Evento ${prazo(diasEvento)}.`;
}
